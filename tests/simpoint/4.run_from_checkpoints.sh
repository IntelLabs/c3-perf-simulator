...
#Need to export SPEC_PATH where spec2017 benchmarks are located
SPEC_PATH=/spec2017
...
if [[ "$CONFIG" == "base" ]]; then
  C3_OPT="--cpu-type=O3_X86_icelake_base -e $GEM5_PATH/c3_no_wrap_enable.env --pointer-decryption-delay 0 --data-keystream-delay 0"
elif [[ "$CONFIG" == "c3" ]]; then
  C3_OPT="--cpu-type=O3_X86_icelake_c3 -e $GEM5_PATH/c3_no_wrap_enable.env --pointer-decryption-delay 3 --data-keystream-delay 4"
elif [[ "$CONFIG" == "c3-predtlb" ]]; then
  C3_OPT="--cpu-type=O3_X86_icelake_c3 -e $GEM5_PATH/c3_no_wrap_enable.env --pointer-decryption-delay 3 --data-keystream-delay 4 --enablePredTLB"
else
...
CHECKPOINT_DIR=$GEM5_PATH/tests/simpoint/output/$BENCHMARK_CODE
...
$GEM5_PATH/build/X86/gem5.opt \
  ...
  --restore-simpoint-checkpoint -r 1 --checkpoint-dir $CHECKPOINT_DIR \
